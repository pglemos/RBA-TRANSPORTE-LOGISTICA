...
export default function FinancePage() {
  ...
  const handleDeletePayment = async (id: string) => {
    if (!confirm('Confirmar exclusão permanente deste lançamento? Esta ação não pode ser desfeita.')) return;
    setErrorMsg('');
    setMsg('');
    try {
      ...
      if (data && data.success) {
        ...
        setErrorMsg(data.error || 'Erro ao excluir: perfil sem permissão de exclusão.');
      }
    } catch (e) {
      setErrorMsg('Erro de rede ao excluir lançamento.');
    }
  };
                ...
                <tbody className="divide-y divide-slate-150 font-semibold text-slate-700">
                  {filteredPayments.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50">
                      ...
                      <td className="p-4 text-[10.5px] text-slate-500">
                      ...
                      <td className="p-4 text-right">
                        {p.status === 'Pendente' ? (
                          ...
                          <div className="flex justify-end items-center gap-1.5">
                            <span className="text-[10px] text-slate-400">{p.status === 'Pago' ? 'Liquidado' : 'Cancelado'}</span>
                            <button
                              type="button"
                              onClick={() => handleDeletePayment(p.id)}
                              title="Excluir lançamento"
                              className="p-1 rounded hover:bg-red-50 text-slate-300 hover:text-red-600 transition-colors cursor-pointer"
                            >
